...
const MIGRATIONS = [
    ...
    },
    {
        ...
        up: async (db) => {
            ...
            const outcomeCorrectExists = await db.get(`SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
         WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'resolution_claims' AND COLUMN_NAME = 'outcome_correct'`, []);
            ...
            const resolvedAtExists = await db.get(`SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
         WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'resolution_claims' AND COLUMN_NAME = 'resolved_at'`, []);
            ...
            const revealValidExists = await db.get(`SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
         WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'resolution_claims' AND COLUMN_NAME = 'reveal_valid'`, []);
            ...
            const settledAtExists = await db.get(`SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
...
async function runMigrations(db) {
  ...
  `);
    ...
    if (n === 0) {
        const faucetExists = await db.get(`SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
    ...
    for (const migration of MIGRATIONS) {
        const already = await db.get(`SELECT id FROM schema_migrations WHERE id = ?`, [migration.id]);
        if (already) {
