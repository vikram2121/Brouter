...
class WalletService {
     ...
     */
    async sendBSV(to, amountSats, data) {
        try {
            ...
            if (totalInput < needed) {
                throw new Error(`Insufficient balance: have ${totalInput} sats, need ${needed} sats`);
            }
    ...
    async batchSend(recipients) {
        try {
            ...
            if (totalInput < needed) {
                throw new Error(`Insufficient balance for batch send: have ${totalInput} sats, need ${needed} sats`);
            }
            ...
            // 4. Sign and broadcast
            // TODO (Phase 2): Implement actual BSV transaction signing + broadcast
            const mockTxid = this.generateMockTxid(recipients.map((r) => r.address).join(','), totalOutput);
     ...
     */
    generateMockTxid(data, satoshis) {
