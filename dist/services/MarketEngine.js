...
const MarketStateLog_1 = require("./MarketStateLog");
class MarketEngine {
    constructor(db) {
        this.db = db;
        this.stateLog = new MarketStateLog_1.MarketStateLogService(db);
    }
    /**
    ...
    async create(input) {
        ...
        if (input.closesAt >= input.resolvesAt)
            throw new Error('closesAt must be before resolvesAt');
        ...
        if (hoursDiff < minDuration)
            throw new Error(`closesAt must be at least ${minDuration} hours from now`);
        if (!input.resolutionCriteria?.trim())
    ...
    async listByState(state, limit = 100) {
        const rows = await this.db.all('SELECT * FROM markets WHERE state = ? ORDER BY createdAt DESC LIMIT ?', [state, limit]);
    ...
    async listByDomain(domain, limit = 100) {
        const rows = await this.db.all('SELECT * FROM markets WHERE domain = ? ORDER BY createdAt DESC LIMIT ?', [domain, limit]);
        return rows.map((r) => this.mapRow(r));
    }
    /**
    ...
    async listOpen(limit = 100) {
        const rows = await this.db.all('SELECT * FROM markets WHERE state = ? ORDER BY closesAt ASC LIMIT ?', ['OPEN', limit]);
        return rows.map((r) => this.mapRow(r));
    }
    /**
    ...
    async transitionState(input) {
        ...
        await this.db.run(`UPDATE markets SET ${setClauses} WHERE id = ?`, values);
