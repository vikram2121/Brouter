...
class MarketService {
    ...
    async create(title, description, domain = 'crypto', tier = 'weekly', closesAt, resolvesAt, resolutionCriteria, oracleProvider = null, oracleMarketId = null, createdBy = null) {
        ...
        if (!['rapid', 'weekly', 'anchor'].includes(tier))
            throw new Error('tier must be rapid, weekly, or anchor');
        const now = new Date();
        if (closesAt <= now)
            throw new Error('closesAt must be in the future');
        if (resolvesAt <= closesAt)
            throw new Error('resolvesAt must be after closesAt');
        if (!resolutionCriteria?.trim())
            throw new Error('resolutionCriteria required');
        ...
        await this.db.run(`INSERT INTO markets (
        ...
        await this.db.run(`INSERT INTO market_state_log (marketId, fromState, toState, triggeredBy, loggedAt)
       VALUES (?, NULL, ?, ?, ?)`, [id, 'PROPOSED', createdBy ?? null, nowStr]);
        return (await this.get(id));
    }
    async list(tier, domain, state, limit = 50) {
    ...
    async getPositions(marketId) {
        const rows = await this.db.all(`SELECT s.id, s.marketId, s.agentId, s.direction, s.amountSats, s.createdAt
       FROM stakes s
       WHERE s.marketId = ?
       ORDER BY s.amountSats DESC`, [marketId]);
        return rows.map((r) => ({
            id: r.id,
            marketId: r.marketId,
            agentId: r.agentId,
    ...
    async takePosition(marketId, agentId, direction, amountSats) {
        if (amountSats < 1)
            throw new Error('Amount must be at least 1 sat');
        if (!['yes', 'no'].includes(direction))
            throw new Error('Direction must be yes or no');
        const market = await this.get(marketId);
        if (!market)
            throw new Error('Market not found');
        if (market.state === 'SETTLED' || market.state === 'ARCHIVED')
            throw new Error('Market has already settled');
        ...
        await this.db.run(`INSERT INTO stakes (id, marketId, agentId, direction, amountSats, oddsAtStake, impliedProbability, consensusAfter, createdAt)
       VALUES (?, ?, ?, ?, ?, 1.0, 0.5, 0.5, ?)`, [id, marketId, agentId, direction, amountSats, now]);
        // Update denormalized market totals
        const col = direction === 'yes' ? 'totalYesSats' : 'totalNoSats';
        await this.db.run(`UPDATE markets SET ${col} = ${col} + ?, agentCount = (
         SELECT COUNT(DISTINCT agentId) FROM stakes WHERE marketId = ?
       ), updatedAt = NOW() WHERE id = ?`, [amountSats, marketId, marketId]);
    ...
    async resolve(marketId, outcome, resolvedBy) {
        const market = await this.get(marketId);
        if (!market)
            throw new Error('Market not found');
        ...
        if (market.state !== 'RESOLVING')
            throw new Error(`Market must be in RESOLVING state, currently ${market.state}`);
        const now = new Date().toISOString().slice(0, 19).replace('T', ' ');
        // Update market with resolution and transition to SETTLED
        await this.db.run(`UPDATE markets SET state = 'SETTLED', resolvedOutcome = ?, settledAt = ?, updatedAt = NOW() WHERE id = ?`, [outcome, now, marketId]);
        // Log state transition to SETTLED
        await this.db.run(`INSERT INTO market_state_log (marketId, fromState, toState, triggeredBy, oracleOutcome, oracleSource, loggedAt)
       VALUES (?, ?, ?, ?, ?, ?, ?)`, [marketId, 'RESOLVING', 'SETTLED', resolvedBy, outcome, 'manual', now]);
    ...
    async open(marketId) {
        ...
        if (market.state !== 'PROPOSED')
            throw new Error(`Market must be in PROPOSED state, currently ${market.state}`);
        const now = new Date().toISOString().slice(0, 19).replace('T', ' ');
        await this.db.run(`UPDATE markets SET state = 'OPEN', openedAt = ?, updatedAt = NOW() WHERE id = ?`, [now, marketId]);
        await this.db.run(`INSERT INTO market_state_log (marketId, fromState, toState, loggedAt) VALUES (?, ?, ?, ?)`, [marketId, 'PROPOSED', 'OPEN', now]);
        return (await this.get(marketId));
    }
    /**
    ...
    async lock(marketId) {
        ...
        if (market.state !== 'OPEN')
            throw new Error(`Market must be in OPEN state, currently ${market.state}`);
        const now = new Date().toISOString().slice(0, 19).replace('T', ' ');
        await this.db.run(`UPDATE markets SET state = 'LOCKED', lockedAt = ?, updatedAt = NOW() WHERE id = ?`, [now, marketId]);
        await this.db.run(`INSERT INTO market_state_log (marketId, fromState, toState, loggedAt) VALUES (?, ?, ?, ?)`, [marketId, 'OPEN', 'LOCKED', now]);
        return (await this.get(marketId));
    }
    /**
     ...
     */
    async startResolution(marketId) {
        const market = await this.get(marketId);
        if (!market)
            throw new Error('Market not found');
        if (market.state !== 'LOCKED')
            throw new Error(`Market must be in LOCKED state, currently ${market.state}`);
        const now = new Date().toISOString().slice(0, 19).replace('T', ' ');
        await this.db.run(`UPDATE markets SET state = 'RESOLVING', resolvingAt = ?, updatedAt = NOW() WHERE id = ?`, [now, marketId]);
        await this.db.run(`INSERT INTO market_state_log (marketId, fromState, toState, loggedAt) VALUES (?, ?, ?, ?)`, [marketId, 'LOCKED', 'RESOLVING', now]);
        return (await this.get(marketId));
    }
    mapRow(r) {
        return {
