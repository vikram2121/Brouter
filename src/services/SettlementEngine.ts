...
export interface SettlementConfig {
  bsvClient?: any       // BSV transaction signer (for future use)
  walletAddress: string // Brouter's BSV address
  walletPrivKey: string // Brouter's private key (WIF format)
...
export class SettlementEngine {
  ...
  constructor(config: SettlementConfig, private db: any) {
  ...
  async settle(
    ...
    await this.db.run(
      `INSERT INTO settlement_dust (marketId, feeSats, roundingDustSats, totalDustSats)
       VALUES (?, ?, ?, ?)`,
      [marketId, feeSats, roundingDustSats, totalDustSats]
    )
  ...
  private async writePayouts(
    ...
    const agentEarnings = new Map<string, bigint>()

    // STEP 1: Update stakes with payouts
    for (const payout of payouts) {
      await this.db.run(
        `UPDATE stakes SET payoutSats = ?, payoutTxid = ? WHERE id = ?`,
        [payout.payoutSats, `pending_${payout.stakeId}`, payout.stakeId]
      )
    ...
    for (const [agentId, earnings] of agentEarnings) {
      await this.db.run(
        `UPDATE agents SET totalEarnedSats = totalEarnedSats + ? WHERE id = ?`,
        [earnings.toString(), agentId]
      )
    }

    // STEP 3: Update market with resolution outcome and TXID
    await this.db.run(
      `UPDATE markets SET outcome = ?, resolutionAnchorTxid = ? WHERE id = ?`,
      [outcome, resolutionTxid, marketId]
    )
   ...
   */
  private async sendPayouts(payouts: Array<{
    stakeId: string
    agentId: string
    direction: 'yes' | 'no'
    amountSats: number
    ...
    const byAgent = new Map<string, typeof winners>()
   ...
   */
  private async updateCalibration(marketId: string, outcome: Outcome): Promise<void> {
    ...
    const signals = await this.db.all(
      `SELECT agentId, claimedProb FROM signals WHERE marketId = ? AND claimedProb IS NOT NULL ORDER BY createdAt ASC`,
      [marketId]
    )
    ...
    const briersByAgent = new Map<string, number>()
    ...
    for (const [agentId, totalBrier] of briersByAgent) {
      ...
      const existing = await this.db.get(
        `SELECT brierScore, marketCount FROM calibration_scores WHERE agentId = ? AND domain = ?`,
        [agentId, market.domain]
      )
      ...
      if (existing) {
        ...
        await this.db.run(
          `UPDATE calibration_scores SET brierScore = ?, marketCount = ?, updatedAt = NOW()
           WHERE agentId = ? AND domain = ?`,
          [newBrierScore, newCount, agentId, market.domain]
        )
      } else {
        // Insert: first time seeing this agent in this domain
        await this.db.run(
          `INSERT INTO calibration_scores (agentId, domain, brierScore, marketCount, updatedAt)
           VALUES (?, ?, ?, 1, NOW())`,
          [agentId, market.domain, totalBrier]
        )
      }
    }
  ...
  private buildTransaction(
    inputs: UTXOInput[],
    outputs: Array<{ address?: string; satoshis: number; script?: string }>
  ): any {
    const tx = {
      version: 1,
   ...
   */
  private signTransaction(tx: any, inputs: UTXOInput[]): any {
    try {
      ...
      for (let i = 0; i < tx.inputs.length; i++) {
        ...
        const sigHash = createHash('sha256').update(sigPreimage).digest()

        // Sign with secp256k1
        const sigBytes = sign(sigHash, privKey)
   ...
   */
  private txidFromSignedTx(tx: any): string {
    try {
      const serialized = this.serializeTransaction(tx)
      const hash1 = createHash('sha256').update(serialized).digest()
   ...
   * Serialize transaction for signing (simplified BIP143)
   */
  private serializeForSigning(tx: any, inputIndex: number, scriptPubKey: string): Buffer {
   ...
   */
  private serializeTransaction(tx: any): Buffer {
    // Stub: Return placeholder
   ...
   */
  private wifToHex(wif: string): string {
  ...
  private async getMarketRow(marketId: string): Promise<any> {
    return this.db.get('SELECT * FROM markets WHERE id = ?', [marketId])
  }

  private async getStakes(marketId: string): Promise<Stake[]> {
